...
class Backend {
	...
	static func fetchWeather(locationId: Int, completion: @escaping (Result<WeatherResponse, Error>) -> Void) {
		Request(path: "/location/\(locationId)/").perform(type: WeatherResponse.self, completion: completion)
	}
